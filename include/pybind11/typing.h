...
struct handle_type_name<typing::Tuple<Types...>> {
    static constexpr auto name = const_name(PYBIND11_TYPE_HINT_TUPLE) + const_name("[")
...
struct handle_type_name<typing::Dict<K, V>> {
    static constexpr auto name = const_name(PYBIND11_TYPE_HINT_DICT) + const_name("[")
...
struct handle_type_name<typing::List<T>> {
    static constexpr auto name = const_name(PYBIND11_TYPE_HINT_LIST) + const_name("[")
                                 + make_caster<T>::name + const_name("]");
};

template <typename T>
struct handle_type_name<typing::Set<T>> {
    static constexpr auto name = const_name(PYBIND11_TYPE_HINT_SET) + const_name("[")
                                 + make_caster<T>::name + const_name("]");
};

template <typename T>
struct handle_type_name<typing::Iterable<T>> {
    static constexpr auto name = const_name(PYBIND11_TYPE_HINT_ITERABLE) + const_name("[")
                                 + make_caster<T>::name + const_name("]");
};

template <typename T>
struct handle_type_name<typing::Iterator<T>> {
    static constexpr auto name = const_name(PYBIND11_TYPE_HINT_ITERATOR) + const_name("[")
                                 + make_caster<T>::name + const_name("]");
};
...
struct handle_type_name<typing::Callable<Return(Args...)>> {
    ...
    static constexpr auto name
        = const_name(PYBIND11_TYPE_HINT_CALLABLE) + const_name("[[")
...
struct handle_type_name<typing::Callable<Return(ellipsis)>> {
    ...
    using retval_type = conditional_t<std::is_same<Return, void>::value, void_type, Return>;
    static constexpr auto name = const_name(PYBIND11_TYPE_HINT_CALLABLE) + const_name("[..., ")
