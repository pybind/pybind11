...
def test_bad_python_to_cpp_casts(m):
    ...
    with pytest.raises(TypeError, match=r'^round_trip_tensor_noconvert\(\): incompatible function arguments'):
...
def test_round_trip(m):
    ...
    with pytest.raises(TypeError, match=r'^round_trip_rank_0_noconvert\(\): incompatible function arguments'):
