...
class DefineBase {
public:
    DefineBase(int value) : value_(value) { track_created(this, value); }
    ...
    virtual ~DefineBase() { track_destroyed(this); }
...
public:
    ...
    DefineBaseContainer(std::shared_ptr<T> obj) : obj_(obj) {}
...
public:
    using T = DefineBase<label>;
    DefineBaseUniqueContainer(std::unique_ptr<T> obj) : obj_(std::move(obj)) {}
...
public:
    ...
    using BaseT::BaseT;
    int value() const override { PYBIND11_OVERLOAD(int, BaseT, value); }
};
...
public:
    PyInstanceStats(ConstructorStats &cstats, py::handle h) : cstats_(cstats), h_(h) {}
    void track_created() {
        cstats_.created(h_.ptr());
        cstats_.value(py::str(h_).cast<std::string>());
    }
    void track_destroyed() { cstats_.destroyed(h_.ptr()); }
...
TEST_SUBMODULE(ownership_transfer, m) {
    ...
    class_shared_<BaseBad>(m, "BaseBad").def(py::init<int>()).def("value", &BaseBad::value);
