...
def test_pybind11_str_raw_str():
    ...
    assert cvt(b'Bytes') == u"Bytes" if info.PY2 else "b'Bytes'"
    ...
    assert cvt(set()) == u"set([])" if info.PY2 else "set()"
    assert cvt({3, 3}) == u"set([3])" if info.PY2 else "{3}"
...
def test_memoryview(method, args, fmt, expected_view):
    ...
    if isinstance(expected_view, bytes) or not info.PY2:
...
@pytest.mark.xfail("info.PYPY", reason="getrefcount is not available")
